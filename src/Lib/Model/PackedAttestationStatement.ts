...
export class PackedAttestationStatement implements IAttestationStatement {
  ...
  public async decode(cborObject: any): Promise<IAttestationStatement> {
    ...
    if ("ecdaaKeyId" in cborObject) {
      this.ecdaaKeyId = cborObject.ecdaaKeyId;
    }
  ...
  public validateSignature(
    ...
    if (this.x5c) {
      const leafCert = this.base64ToPem(
      ...
      if (
        !certificateInfo.subject.C ||
        certificateInfo.subject.C.length !== 2
      ) {
        throw new Error(
      ...
      const hashingAlgorithm = CoseAlgorithm.getByIdentifier(
        publicKeyCose.$algorithm
      );
